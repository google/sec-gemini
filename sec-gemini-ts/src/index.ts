...
import {
    ...
    PublicUser,
    UserInfo,
    ModelInfo, // Important: Use this for model representation
...
class SecGemini {
  ...
  private stableModel: ModelInfo | null = null;
  private experimentalModel: ModelInfo | null = null;
   ...
   */
  public static async create(apiKey: string = "", baseURL: string = "", websocketsURL: string = ""): Promise<SecGemini> {
  ...
  private processAvailableModels(models?: ModelInfo[]): void {
    ...
    for (const model of models) {
        if (model.is_experimental) {
            if (!this.experimentalModel) { // Take the first experimental model found
                this.experimentalModel = model;
                console.info(`Found experimental model: ${model.model_string}`); // Use logger
            } else {
                 console.warn(`Multiple experimental models found. Using the first one: ${this.experimentalModel.model_string}`); // Use logger
            }
        } else {
             if (!this.stableModel) { // Take the first stable model found
  ...
  /**
   * Gets the identified stable ModelInfo object.
   * @returns The stable ModelInfo object or null if none was found/available.
   */
  public getStableModel(): ModelInfo | null {
    return this.stableModel;
  }

  /**
   * Gets the identified experimental ModelInfo object.
   * @returns The experimental ModelInfo object or null if none was found/available.
   */
  public getExperimentalModel(): ModelInfo | null {
   ...
   * @param options.model - The model to use. Can be 'stable', 'experimental', or a specific ModelInfo object. Defaults to 'stable'.
  ...
  public async createSession(options: {
    ...
    description?: string;
    logSession?: boolean;
    model?: 'stable' | 'experimental' | ModelInfo;
    language?: string;
    ...
    if (typeof model === 'string') {
      if (model === 'stable') {
        ...
        throw new Error(`Invalid model identifier string: '${model}'. Use 'stable', 'experimental', or provide a ModelInfo object.`);
      }
    } else if (typeof model === 'object' && model !== null && model.model_string) {
        // Basic check if it looks like a ModelInfo object
        // More robust validation could be added if needed
         ...
         const modelIsAvailable = this.userInfo?.available_models?.some(m => m.model_string === resolvedModel.model_string);
         if (!modelIsAvailable) {
             console.warn(`Using provided ModelInfo for '${resolvedModel.model_string}', but it wasn't listed in available models.`); // Use logger
         }
         if (resolvedModel.is_experimental && !this.user.allow_experimental) {
             throw new Error(`Cannot create session with provided experimental model '${resolvedModel.model_string}': User is not authorized.`);
         }
    }
     else {
        throw new Error("Invalid model parameter provided. Use 'stable', 'experimental', or a ModelInfo object.");
    }
