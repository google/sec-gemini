...
class InteractiveSession {
  ...
  public get description(): string {
   ...
   public get create_time(): number | undefined {
    ...
    public get messages(): ReadonlyArray<Message> { // Return ReadonlyArray
        if (!this._session) throw new Error("Session is not initialized.");
    ...
    public get files(): ReadonlyArray<PublicSessionFile> { // Return ReadonlyArray
         if (!this._session) throw new Error("Session is not initialized.");
  ...
  public async register(options: {
    ...
    try {
        ...
        resp = await this.http.post<OpResult>(
    ...
    console.info(
      `[Session][Register][API]: Session ${sessionId} (${sessionName}) registered successfully.`
    );
   ...
   */
  public async resume(sessionId: string): Promise<boolean> { // Keep bool return for compatibility? Or change to void? Change to void is cleaner.
    if (!sessionId) {
    ...
    console.info(
        `[Session][Resume]: Session ${this.id} (${this.name}) resumed successfully.`
    );
  ...
  public async generate(prompt: string): Promise<SessionResponse> {
    ...
    try {
        ...
        resp = await this.http.post<SessionResponse>(
  ...
  public async update(
    ...
    ttl: number = 0
      ...
      console.info(
          `[Session][Update][API]: Session ${this.id} updated successfully.`
      );
  ...
  public async attachFile(
    ...
    console.info(
      `[Session][Attachment][API]: File ${filename} attached to session ${this.id}.`
    );
  ...
  public async detachFile(filename: string): Promise<void> {
    ...
    console.info(
      `[Session][DetachFile][API]: File ${filename} detached from session ${this.id}.`
    );
  ...
  private async _fetchSessionById(sessionId: string): Promise<PublicSessionOutput> {
    ...
    try {
      // Ensure EndPointsEnum.GET_SESSION maps to '/v1/session/get'
       respData = await this.http.get<PublicSessionOutput | null>(
  ...
  public async visualize(): Promise<void> {
    ...
    function printNode(messageId: string, indent: string = "") {
        ...
        if (msg.mime_type?.startsWith('text/')) {
            contentPreview = (msg.content || '').substring(0, 80).replace(/\n/g, ' ') + ( (msg.content?.length || 0) > 80 ? '...' : '');
