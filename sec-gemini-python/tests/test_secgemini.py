...
def test_get_experimental_model(secgemini_client_with_models, mock_experimental_model_info):
...
def test_resume_session(secgemini_client_with_models, httpx_mock, mock_public_session):
    ...
    session = secgemini_client_with_models.resume_session(session_id=mock_public_session.id)
...
def test_create_session_invalid_model_name(secgemini_client_with_models):
    with pytest.raises(ValueError, match="Invalid model name invalid_model - must be 'stable' or 'experimental'."):
...
def test_create_session_invalid_model_type(secgemini_client_with_models):
    with pytest.raises(ValueError, match="Invalid model 123 - must be a ModelInfo object."):
...
def test_init_no_api_key():
    with pytest.raises(ValueError, match="API key required"):
        SecGemini(api_key="", base_url='http://localhost:8000', base_websockets_url='ws://localhost:8000')

def test_init_invalid_base_url(mock_user_info_with_models):
    with pytest.raises(ValueError, match="Invalid base_url"):
        SecGemini(api_key="test_key", base_url='invalid_url', base_websockets_url='ws://localhost:8000')
...
def test_init_invalid_websockets_url(mock_user_info_with_models):
    with pytest.raises(ValueError, match="Invalid base_websockets_url"):
        SecGemini(api_key="test_key", base_url='http://localhost:8000', base_websockets_url='invalid_ws_url')
...
def test_init_get_info_fails(httpx_mock):
    ...
    with pytest.raises(ValueError, match="API Key is invalid or the API is down."):
        print('hi')
        SecGemini(api_key="test_key", base_url='http://localhost:8000', base_websockets_url='ws://localhost:8000')
...
def test_get_info_request_error(secgemini_client_with_models, httpx_mock):
    ...
    httpx_mock.add_response(
        ...
        json={"detail": "Authentication credentials were not provided or were invalid."}
    )
