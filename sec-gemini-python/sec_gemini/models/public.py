...
class PublicSessionFile(BaseModel):
    ...
    sha256: str = Field(..., title="SHA256 of the file", description="SHA256 of the file.")
    mime_type: str = Field(..., title="Mime type", description="Mime type.")
    content_type_label: Optional[str] = Field(None, title="Content type label", description="Content type label.")
...
class PublicSession(BaseModel):
    ...
    id: str = Field(
        default_factory=lambda: uuid4().hex, title="Session ID", description="Session unique ramdom identifier."
    )

    user_id: str = Field(..., title="User ID", description="The user ID this session belongs to.")

    org_id: str = Field(..., title="Organization ID", description="The organization ID this session belongs to.")

    model: ModelInfo = Field(..., title="Model", description="Model configuration used in the session.")

    ttl: int = Field(..., title="Time to Live", description="The time to live of the session in seconds.")

    language: str = Field("en", title="Language", description="The iso-code of the session language.")

    turns: int = Field(default=0, title="Number of Turns", description="The number of turns in the session.")

    name: str = Field(..., title="Session Name", description="Human readable session name.")
    ...
    num_messages: int = Field(
        default=0, title="Number of Messages", description="The number of messages in the session."
    )

    messages: list[Message] = Field(
        default_factory=list, title="Messages", description="The list of messages comprising the session so far."
    )

    usage: Usage = Field(default_factory=Usage, title="Usage", description="Session usage statistics.")

    can_log: bool = Field(..., title="Can Log", description="Whether the session can be logged or not.")

    state: State = Field(State.START, title="State", description="The state the session belongs to.")

    files: list[PublicSessionFile] = Field(
        default_factory=list, title="Files", description="The list of files uploaded to the session."
    )
