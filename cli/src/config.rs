...
impl Config {
    ...
    /// Creates a frozen value (delete and persist are ignored).
    ...
    pub fn force(&mut self) {
        if let ConfigImpl::Unknown { desc } = self.0 {
            self.0 = read_value(desc);
        }
    }

    /// Deletes the config of a known value read from config (does nothing otherwise).
    pub fn delete(&self) {
        if let ConfigImpl::Known { path, config: true, desc, .. } = &self.0 {
            let prompt = desc.prompt;
            log::info!("Deleting {prompt} config file.");
...
enum ConfigImpl {
    ...
    Frozen {
        /// The value read from the command line, environment, terminal, or config file.
        value: String,
    },
}
